...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i + CHUNK_SIZE <= samples.size(); i += CHUNK_SIZE) {
		...
		if(frame_count >= 200 && frame_count <= samples.size()-200){
			if(frame_count % 5 == 0){		
				for (float magnitude : magnitudes) {
					magnitude = sqrt(clip_float(magnitude));
					quantized_magnitudes.push_back(static_cast<uint32_t>(magnitude * (UINT32_MAX-256)));
				}
			}
		}
		//printf("\n");
